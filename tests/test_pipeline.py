...
def test_first_run_pushes_default_and_parts(make_settings, fake_wbs, fake_langflow, workcode):
    ...
    assert state.targets["part:sales"].langflow_name == f"{BASE}_sales"

    entries = changelog.read(settings.changelog_file)
    assert {e["target"] for e in entries} == {"default", "part:it", "part:sales"}
    assert all(e["langflow_name"] for e in entries)
...
def test_orphan_cleanup_removes_deleted_part(make_settings, fake_wbs, fake_langflow, workcode):
    ...
    removed = [e for e in changelog.read(settings.changelog_file) if e["status"] == "removed"]
...
def test_orphan_guard_skips_cleanup_when_departments_empty(make_settings, fake_wbs, fake_langflow, workcode):
    ...
    state = state_store.load(settings.state_file)
    assert "part:sales" in state.targets  # preserved despite being "gone"
...
def test_failure_marks_targets_failed_but_advances_newest(
    ...
    # newest files still advanced (per the promote-first design)
    assert settings.data_path_for(BASE).exists()
    assert settings.data_path_for(f"{BASE}_sales").exists()
