...
class Settings(BaseSettings):
    ...
    langflow_file_name: str = Field("wbs", description="Base name of the uploaded file (no extension)")
    ...
    def data_file(self) -> Path:
        """The single canonical 'newest' data file at rest."""
        return Path(self.state_dir) / f"{self.langflow_file_name}.json"

    @property
    def temp_file(self) -> Path:
        """Transient candidate file written each tick, compared then promoted/deleted."""
        return Path(self.state_dir) / f"{self.langflow_file_name}.tmp.json"
