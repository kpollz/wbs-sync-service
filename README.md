...
4. **Compare** — băm SHA-256 nội dung temp rồi so với hash của file **newest** `data/wbs.json` (đọc lại từ disk — compare "trước/sau"):
   ...
   - **Khác** (hoặc `--force`) → có đổi → tính **diff** (thêm/sửa/xóa) → upload.
5. **Upload (retry)** — `langflow_client.replace_file(temp)` với **retry** (`SYNC_MAX_RETRIES`, backoff tuyến tính). Mỗi lần: `GET /api/v2/files` → `DELETE` hết file trùng tên → `POST /api/v2/files` upload temp.
6. **Finalize**:
   - **Thành công** → promote temp → newest (atomic `os.replace`, temp bị "ăn" luôn) → ghi 1 dòng **changelog** (`success`) + cập nhật `state.json`.
   - **Thất bại** (hết retry) → **xóa temp**, giữ nguyên newest → ghi 1 dòng **changelog** (`failed`) + đánh dấu `state.last_status=failed`. Tick kế tiếp (6h sau) sẽ thấy vẫn còn khác newest → tự upload lại.
...
- **Retry**: upload fail → thử lại tối đa `SYNC_MAX_RETRIES` lần, mỗi lần cách `SYNC_RETRY_BACKOFF * attempt` giây. Hết retry → ghi dòng `failed`, tick sau tự làm lại.
